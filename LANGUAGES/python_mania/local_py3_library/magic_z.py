...
class MagicBall(list):
    ...
    def type_responser(self, _input, **params):
        ...
        for _type, responser in type_mapper.items():
    ...
    #TODO 此方法在py2下有效，在py3下无效，具体原因待查
    # 似乎是globals的机理发生了变化。
    def str_to_global_var(self, string, obj):
        ...
        #TODO  This line adds for TEST only
        return globals()
...
def str_to_global_var_func(string, obj):
    """将传入的有效string变为全局变量，值为obj。
       TODO: 需要考虑string不是有效变量名的情况。
    """
    command_1 = 'global %s' % string
